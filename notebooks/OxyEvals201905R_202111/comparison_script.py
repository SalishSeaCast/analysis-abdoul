...
def main(config_file, obs_data_code, year, quarter):
    ...
    elif obs_data_code == 'ctd_from_dfo':
        df1 = pd.read_pickle(f'{config["ctddir2"]}/dfo/ctd/{start_date:%Y}.p')
        df1 = df1.rename(columns={"lat": "Lat", "lon": "Lon", "time": "dtUTC", "z": "Z", "DO (uM)": "Oxygen_Dissolved"})
    elif obs_data_code == 'bot_from_dfo':
        df1 = pd.read_pickle(f'{config["ctddir2"]}/dfo/bottle/{start_date:%Y}.p')
        df1 = df1.rename(columns={"lat": "Lat", "lon": "Lon", "time": "dtUTC", "z": "Z", "DO (uM)": "Oxygen_Dissolved"})
    elif obs_data_code == 'bot_from_dfo2':
        df1 = pd.read_csv(f'/ocean/atall/MOAD/Obs/DFO_OcProfDB.csv', parse_dates=['dtUTC'])
    elif obs_data_code == 'bot_from_ncei':
        df1 = pd.read_pickle(f'{config["ctddir2"]}/ncei/{start_date:%Y}.p')
        df1 = df1.rename(columns={"lat": "Lat", "lon": "Lon", "time": "dtUTC", "z": "Z", "DO (uM)": "Oxygen_Dissolved"})
    elif obs_data_code == 'cioos_from_file':
        df = pd.read_csv(f'{config["cioosdir"]}IOS_BOT_Profiles_58f5_df59_9c39.csv')
        df['dtUTC'] = pd.to_datetime(df['dtUTC']).dt.strftime('%Y-%m-%dT%H:%M:%S')
        df.to_csv('/ocean/atall/MOAD/Obs/CIOOS/IOS_BOT_Profiles_2018-2023.csv')
        df1 = pd.read_csv(f'{config["cioosdir"]}IOS_BOT_Profiles_2018-2023.csv', parse_dates=['dtUTC'])
        df1 = df1.rename(columns={"latitude": "Lat", "longitude": "Lon", "depth": "Z",
                                  "sea_water_temperature": "Temperature", "sea_water_practical_salinity": "Salinity",
                                    "DOXYZZ01": "Oxygen_Dissolved", "NTRZAAZ1": "NO3", "CPHLFLP1": "Chl"})
    elif obs_data_code == 'bb1_from_file':
#        df = pd.read_csv(f'{config["obsdir"]}bb1_bop_level1_0b98_e7e1_e252.csv')
#        df['dtUTC'] = pd.to_datetime(df['time'][1:]).dt.strftime('%Y-%m-%dT%H:%M:%S')
#        df.to_csv('/ocean/atall/MOAD/Obs/bb1/bb1_bop_level1_2010-2023.csv')
        df1 = pd.read_csv(f'{config["obsdir"]}bb1/bb1_bop_level1_2010-2023.csv', parse_dates=['dtUTC'])
        df1['Z'] = df1['depth']
        df1 = df1.rename(columns={"latitude": "Lat", "longitude": "Lon",
                                    "DOXMZZ01": "Oxygen_Dissolved", "NTRZAAZ1": "N"})

    else:
        print ('ERROR, specify ctd, bot,  psf, psfts, pug, pugts, hplc, ferry, ferry_from_file, ferry_file_only, onc, onc_from_file, ctd_from_dfo, ctd_from_dfo, bot_from_ncei, onc_file_only as second argument')
